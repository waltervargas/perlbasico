...
La definición de la subrutina es global; sin hechicería no hay subrutina
...
cálculo que se encuentre de último en una subrutina es automáticamente el valor
de retorno. 
...
Ahora supongamos que agrega una línea más a la subrutina, por ejemplo: 
...
cuando agrega código adicional a una subrutina, puesto que la última expresión
evaluada va a ser el valor de retorno. 

Tome en cuenta que no es la última línea de la subrutina, es la última
...
La subrutina llamada larger_of_fred_or_barney puede ser más útil si no
...
subrutina puede acceder a esta variable para determinar o el número de
...
obtendrá C<undef> si mira más allá del final del array C<@_>. En este mismo
capítulo veremos como hacer esta subrutina más eficiente. 
...
punto y coma después de la expresión del valor de retorno. Aunque Perl permite
...
La subrutina en el ejemplo previo puede ser más simple aún. Debe haber notado
...
las subrutinas van a comenzar con una línea muy parecida a esta, nombres entre
paréntesis. Cuando vea esta línea,  va a saber que la subrutina espera dos
...
anterior no sea visible en la pantalla, usted tipea una línea para incrementar
la variable. 
...
eres simplemente inteligente, vas a usar cada variable más de una vez, y Perl
...
La mayoría de la gente cree que si un programa es más largo que la pantalla,
...
cuando el valor de retorno no es la última expresión evaluada de la subrutina.
...
Pero no ponga la declaración de la subrutina después de la llamada, o el
...
vez que llamamos a la subrutina va a re-definir los valores una vez más. Con
C<state>, podemos tener variables privadas del ámbito de una subrutina pero
...
Esto nos dará un error que sugiere que podríamos usar esto en una versión
futura de Perl: 
