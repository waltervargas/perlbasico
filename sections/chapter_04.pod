...
bucle C<while>. Así, dentro del cuerpo del bucle, vemos línea, una tras otra en
la línea C<$>. N<Usted probablemente ha notado que nosotros nunca mordió esa
entrada. En este tipo de bucle, en realidad no se puede poner chomp en la 
expresión condicional, por lo que a menudo es el primer elemento en el cuerpo 
del ciclo, cuando es necesario. Vamos a ver ejemplos de ese en la siguiente
sección.> Esto es algo que usted querrá hacer con bastante frecuencia, por lo que
naturalmente, Perl tiene un acceso directo para ello. El acceso directo es el siguiente:
