...
\vspace{-6pt}
\normalsize
El ejemplo anterior usa paréntesis como delimitador, pero Perl actualmente permite que uses cualquier carácter de puntuación como delimitador. Algunos ejemplos mas comunes son:
...
\vspace{-6pt}
\normalsize
Podemos referirnos a toda una matriz a través de una notación simple que posee Perl. Solo tiene que usar una arroba (@) antes del nombre del array (y sin corchetes de indices) para referirse a toda la matriz. Este símil funciona a ambos lados del operador de asignación.

\vspace{-6pt}
\small
\begin{Verbatim}[commandchars=\\\{\},numbers=left]
    @rocks  = qw/ bedrock slate lava /;
    @tiny   = ( );                      # una lista vacía 
...
\vspace{-6pt}
\normalsize
Para poder hacer esto, deberá usar comillas simples o escapar el carácter @.
...
En un capítulo anterior vimos que STDIN en un contexto escalar devuelve la ultima linea leída de la entrada estándar. En contexto de lista, este operador devuelve todas las lineas desde el principio hasta el f\mbox{}inal del archivo. Cada linea es retornada como un elemento separado de la lista. Por ejemplo:

\vspace{-6pt}
\small
\begin{Verbatim}[commandchars=\\\{\},numbers=left]
    @lines = <STDIN>; # lee la entrada estándar en el contexto de lista
\end{Verbatim}
\vspace{-6pt}
\normalsize
Cuando la entrada estándar es un archivo, esto va a leer el archivo completo hasta el f\mbox{}inal. Pero cuando se lee de la entrada estándar en Unix, debe usar Control + D.
