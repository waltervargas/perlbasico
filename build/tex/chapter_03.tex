...
Puede invocar o llamar una subrutina desde cualquier expresión usando el nombre de la subrutina (con el signo \&) \footnote{Y frecuentemente un par de paréntesis, incluso si estan vacios}.
...
\vspace{-6pt}
\normalsize
Una vez que lista es enviada a la subrutina, va a estar disponible en la subrutina para hacer lo que sea necesario. Por supuesto, usted tiene que guardar esta lista en algún lugar, Perl automáticamente la lista de parámetros (este es otro nombre para la lista de argumentos) en una variable de tipo array especial llamada \texttt{@\_} durante el tiempo de existencia de la subrutina. La subruina puede acceder a esta variable para determinar o el número de argumentos y el valor de los argumentos.
...
\vspace{-6pt}
\normalsize
Una sola sentencia crea las variables privadas y def\mbox{}ine sus valores. Casí todas las subrutinas van a comenzar con una linea muy parecida a esta, nombres entre paréntesis. Cuando vea esta linea, va a saber que la subrutina espera dos parámetros escalares, que vas a llamar \$m y \$n dentro de la subrutina.

\section{Longitud de la lista de parámetros}

\label{Longitud-de-la-lista-de-par-metros}

En el mundo real, las subrutinas no poseen un valor arbritario para la longitud de la lista de parámetros. Esto es porque en Perl no hay límites innecesarios. Es agradable que Perl sea tan f\mbox{}lexible, pero puede haber problemas cuando se llama a la subrutina con una cantidad diferente de parámetros que espera el autor.
...
Con \texttt{my} podemos hacer variables privadas en una subrutina, sin embargo cada vez que llamamos a la subrutina va a re-def\mbox{}inir los valores una vez mas. Con \texttt{state}, podemos tener variables privadas del ambito de una subrutina pero Perl va a mantener sus valores entre las llamadas.
...
\vspace{-6pt}
\normalsize
Ahora que debemos usar \texttt{strict}, el uso de la variable gobal \$n no esta permitido. No podemos hacer de \$n una variable léxica con \texttt{my} porque entonces no va a mantener el valor.
