...
llamada a C<open>. En general, es mejor cerrar cada filehandle pronto despuÃ©s
...
aunque no estÃ¡n relacionadas con los procesos de I/O nos va a permitir salir de
...
la función eval, que vamos a ver mas adelante en otro capítulo>. Esta
...
sistema se niega a hacer algo que le estamos pidendo, en C<$!> esta la razon
del porqué.  

Hay otra cosa interesante que C<die> puede hacer por nosotros, automáticamente
va a indicar el nombre del programa y el número de linea en donde ocurrio el
error. 
...
colocÃ¡ndolo inmediatamente despuÃ©s de la palabra reservada pero antes de la
lista de argumentos: 
...
buffer despues de cada operación de salida. Entonces si se quiere asegurar que
...
automáticamente. Y tambien digimos que no debería reusar uno de los seis
nombres de filehandles por defecto, al menos que quiera hacer algo especial.
Adicionalmente digimos que los mensajes de error en Perl van a STDERR. Si usa
...
C<print>, con la diferencia de que agrega al final un caracter de nueva linea
...
Esta es una nueva caracteristica de Perl 5.10, y nosotros la usamos solo cuando
...
=item 1. Escriba un programa que actué como C<cat>, pero que invierta el orden
...
prueba2 desde la última linea hasta la primera y luego el archivo prueba desde
...
caracteres, imprima una linea guia con los números de las columnas. Asegúrese
...
el numero del tamaño de la columna en la primera linea, y luego en las otras
